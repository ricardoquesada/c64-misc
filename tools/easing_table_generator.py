...
def run(formula, steps, maxvalue, reverse, absolute, freq):
    ...
    for i in xrange(steps):
        ...
        ret = fn(freq*(i+1)/steps % 1, *args)
        l.append(convert_value(ret, maxvalue))

    # print 8 elements per line
    sys.stdout.write('; autogenerated table: %s -s%s -m%s -a%s -f%s %s%s ' % (os.path.basename(sys.argv[0]), steps, maxvalue, absolute, freq, '-r ' if reverse else '', formula))
...
if __name__ == "__main__":
    ...
    argv = sys.argv[1:]
    try:
        opts, args = getopt.getopt(argv, "lrs:m:a:f:", ["listformulas", "reverse", "tablesize=", "maxvalue=", "absolute=", "freq="])
        for opt, arg in opts:
    ...
    run(formula, steps, maxvalue, reverse, absolute, freq)
